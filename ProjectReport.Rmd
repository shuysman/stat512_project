...
\begin{align}
  ...
  Unit_i &\overset{iid}{\sim} \mathcal{N}(0, \sigma^2_{Site}) \\
  \epsilon_{ij} &\overset{iid}{\sim} \mathcal{N}(0, \sigma^2_{error}) \\
  \begin{split}
    \mu_{ij} &= \beta_0 + \beta_1*AET + \beta_2* CWD + \beta_3 * I_{Unit = WC} + \beta_4*I_{Unit = EC} \\
    &+ \beta_5*I_{Unit = WI} + \beta_6*I_{Unit = BT} + ....\beta_i*climate\ variables
  \end{split}
  (\#eq:full)
\end{align}
\begin{align}
...
\begin{align}
    ...
    I_{unit =BT} =
    \begin{cases}
      1, & \text{if}\ unit = Beartooth \\
      0, & \text{otherwise}
    \end{cases}
\end{align}
```

We ran the saturated cubic model without interactions (Equation \@ref(eq:full)), and then refined it using backwards selection from the $step()$ function and a AIC cutoff of AIC<2. After running the model initially, we found that the residuals had a lot of funneling. Because of this, we chose to log transform the response variable ($growth\_rate$) to reduce this skew. However, log transforming does result in a non-random distribution of residuals. All analysis was done use R statistical software (v4.2.2; R Core Team 2021). 
...
Steve spearheaded the data wrangling and analysis, while Parker took lead on the write-up and formatting. Both people contributed to the model selection, interpretation, discussion, and critique. 
