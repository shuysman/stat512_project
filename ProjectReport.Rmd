...
At each site, data on seedling heights (used to estimate growth rate) and competing plant species was recorded. Water balance variables were estimated using Daymet as inputs for temperature and precipitation. The following predictor variables were evaluated:
...
$\mu${$growth_{rate} | AET + PET + PPT + T_{max} + Micro + Comp\_number + PICO + PIEN + ABLA + random (Unit)$} = XXXXX (\#eq:full)


## Results/Summary of Statistical Findings
The initial full model, with all variables of interest, had an AICc of 1126.504 , and our final model has an AICc of 1122.34 (Table (\@ref(label:modelTable))
