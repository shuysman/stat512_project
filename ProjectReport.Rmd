...
Over the past 40 years, the US Forest Service and National Park Service has planted more than 1500 acres of WBP in the GYE. This study investigated five planting units (Figure \@ref(fig:fig1)) that each contained between two and eight planting sites. This study used a hierarchical sampling design including 5 planting units, with a total of 29 planting sites across units, and thousands of whitebark pine seedlings planted at each planting site, from which a proprionate number of tree seedlings was sampled based on planting site size. (See Figure \@ref(fig:fig2) for model design.)

```{r fig2, fig.cap = "Hierarchical study design with fixed and random effects.  Only variables included in our reserach question are addressed here.  The original study collected additional data, see original paper for details (Laufenberg et al. 2020)", echo = FALSE, out.width = '100%', fig.align='center'}
knitr::include_graphics("measurement_structure_diagram.png")
```

For the 5 larger planting units, name of unit was recorded.  Due to The 29 sites sampled were unevenly spread across those units, in an unbalanced study design.  For each site, a historical gridded daily climate data set (Daymet, Thornton et al. 2016) was used to estimate temperature and precipitation values, which were aggregated into season and annual averages (annual_tmax, annual_tmean, spring_snow, spring_rain).  
...
At each seedling, data was collected on the physical and topographic characteristics of the growing environment (point coordinates, elevation, aspect, and slope) as well as on the competition with other tree species (number of competitors, presence/absence of specific competitor species).  Topographic data was used with the site-level climate data to model water balance variables at the tree-level (aet, pet, cwd).  (For a full list of data collected at each location, see Figure \@ref(fig:fig2)

A variety of data were collected at different scales. The variables that are most pertinent to the research question are: 
\singlespacing
- $growth\_rate$ (*cm/year*): WBP seedling height is used as a proxy for growth rate. This is a continuous response variable and is used as a proxy for seedling growth.  

- $AET$ (*mm*): This is a continuous predictor variable for annual evapotranspiration, the water loss through transpiration by plants on a site, given prevailing water availability. This measurement indicates the magnitude and duration of conditions favorable for plant growth on a site.  This was estimated using a water balance model using historical climate data from Daymet (Thornton et al. 2016) for coordinates taken at each seedling.   

- $CWD$ (*mm*): This is a continuous predictor variable for climatic water deficit, which measures the evaporative water demand that is not met the water supply at a site.  It reflects the drought stress experienced by plants.  It was esimated using the same climate modeling techniques used to estimate AET for coordinates taken at each seedling.

- $Micro$ (*1/0*) - This was a binary variable indicating presence of favorable microsite conditions. 1 if there was a rock or other topographical feature that changed the environmental conditions where the seedling lived.  
\doublespacing
...
```{r site_var, echo = FALSE, eval = TRUE, warnings = FALSE, fig.cap="Growth rate compared with AET for each study site, by planting unit"}
aet_x_growthrt
```
...
We ran the saturated cubic model without interactions (Equation \@ref(eq:full)), and then refined it using backwards selection from the $step()$ function and a AIC cutoff of AIC<2. 
...
After running the model initially, we found that the residuals had a lot of funneling (Figure \@ref(fig:diagnostics)), and we found that log transforming the response variable ($growth\_rate$) reduced this skew. However, log transforming does result in a non-random distribution of residuals, so we tried a variety of different functional transformations to both the response and predictor variables but were unable to reduce this violation. Following the original study (Laufenberg et al. 2020), log transformations were performed with base 10.  All analysis was done use R statistical software (v4.2.2; R Core Team 2021). 
...
The final model suggests that WBP seedling growth is most correlated with the annual evapotranspiration and the number of competitors around the seedling. For a one mm change in AET, there is a multiplicative change in the median seedling growth rate by `r 10^(0.001446)` mm (95% CI: -2.64e-05 to 2.94e-03, profile-likelihood CI **NEED TO UPDATE TO 10^)** and for a one mm change in CWD, there is a multiplicative change in the median seedling growth rate by `r 10^(-9.446e-05)` mm (95% CI: -5.25e-04 to 3.373e-04, profile-likelihood CI), after accounting for number of competitors, unit, and a random effect of site. 
```{r effects, fig.caption = "Mean log growth on effects"}
effect_plot
```

There is a difference in growth rate by unit, which can be visualized in the effects plot ((Figure \@ref(fig:effects)). After accounting for AET, water deficit, and number of competitors, the estimated correlation between any two seedlings in the same site $\hat{ICC}$ = `r round(0.1019/(0.1019 +0.3679),3)`.
