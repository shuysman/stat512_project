...
Over the past 40 years, the US Forest Service and National Park Service has planted more than 1500 acres of WBP in the GYE.  This study investigated five planting units (figure \@ref(fig:fig1)) that each contained between two and eight planting sites. 
...
From examination of the raw data, there appears to be unequal variance in growth rate by planting unit. This suggests that a log transformation be applied, and the variances look better after transformation (Figure \@ref(fig:fig4)).

```{r fig4, echo = FALSE, eval = TRUE, warnings = FALSE, fig.cap="Growth rate by unit, before and after log transformation"}
fig4_combined
```
...
Our reponse variable is `log_growth_rt`.  This is the log of the annual growth rate, determined by measuring seedlings and dividing by the number years since planting.
...
<<<<<<< HEAD
*Fix correlation plot with ggplot. In general, Correlation matrix of predictor variables. Larger circles and darker colors indicate a higher degree of correlation.*

After data were selected based on their correlation, we looked at linearity and potential other polynomial forms (Figure \@ref(fig:pairsEDA)).
```{r pairsEDA, fig.cap = "Raw data visualization of variables of interest", out.width = '60%', fig.align='center', echo = FALSE}
=======
...
>>>>>>> f091177 (add theoretical model for full model)
pairs_plot
```
...
We ran the saturated cubic model without interactions \@ref(eq:full), and then refined it using backwards selection from the $step()$ function and a AIC cutoff of AIC<2. All analysis was done use R statistical software. 

<<<<<<< HEAD

\begin{math}
$$\mu \{log(growth\ rate) | AET, PET, Annual\_P, T_{max}, \\ 
Micro , Comp\_number , PICO , 
PIEN , ABLA , random (Unit)\} \\
= AET^3 + PET^3 + Comp\_number^3 + annual\_P^3 + annual\_Tmax^3+ PIEN+ PICO + ABLA + Micro + (1|unit) $$ (\#eq:full)
\end{math}

*For simplicity sake, lower-order terms were not explicitly written out here but were included in model.*
=======
...
```{r modelTable, include = FALSE, echo = FALSE, warnings = FALSE, messages = FALSE, fig.cap="Model table"}
modelTable
```
...
\begin{math}
$$\mu \{log(growth\ rate) | AET, PET, Annual\_P, T_{max},\\ Comp\_number , PICO, ABLA , random (Unit)\}\\ = AET^3 + PET^3 +
Comp\_number^3 + annual\_P^3 + annual\_Tmax^2+ PICO + ABLA +(1|unit)$$ (\#eq:final)
\end{math}
